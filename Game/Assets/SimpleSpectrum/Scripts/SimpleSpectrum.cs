...
#region COLOR PROPERTIES
    ...
    public void RebuildSpectrum()
    {
        ...
        sprites = new SpriteRenderer[barAmount];
        ...
        {
            GameObject barClone = Instantiate(barPrefab, transform, false); //create the bars and assign the parent
            ...
            SpriteRenderer spr = barClone.transform.GetChild(0).GetComponent<SpriteRenderer>();
            sprites[i] = spr;
            if (rend != null)
            {
...
#endif
            for (int i = 0; i < bars.Length; i++) //applySpect(i);
//            Parallel.For(0,bars.Length,applySpect);
//            void applySpect(int i)
            {
                {
...
/**/                    bar.localScale = new Vector3(barXScale, newYScale, 1);
                    //                int temp = getHSL(180, Math.Sin(hue + frame / 40) * 100, 100);

                    Color tempColor = Color.HSVToRGB(0.5F, (float)Math.Sin(Game.time), 1F);
                    tempColor.a = Math.Min(newYScale, 0.1F);
/**/                    sprites[i].color = tempColor; //new Color(1,1,1,0.2F+newYScale);
